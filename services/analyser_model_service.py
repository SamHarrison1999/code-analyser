...
# This service exposes your multi-label classifier over HTTP and adds an optional rationale generator and a separate /explain endpoint.
# This file exposes your fine-tuned RoBERTa model (multi-label) over HTTP for the Code Analyser to call.
from typing import List, Dict, Any, Optional

# FastAPI provides a minimal, fast HTTP layer for inference.
# We use FastAPI for a lightweight REST API.
from fastapi import FastAPI

# Pydantic validates request/response schemas for a stable contract.
# Pydantic models define stable request/response shapes.
from pydantic import BaseModel, Field

# Auto* loaders pick the correct classes from the on-disk config.
# We rely on Hugging Face Transformers to load the exported artefacts.
from transformers import AutoTokenizer, AutoModelForSequenceClassification, AutoModelForSeq2SeqLM

# Torch provides device placement and sigmoid for multi-label probabilities.
# Torch handles tensors, device placement, and the sigmoid we need for multi-label output.
import torch

# We use os/pathlib to resolve paths and read optional environment configuration.
...
# requests allows optional LLM API calls for /explain when no local rationale model is configured.
import requests

import json

# Helpful metadata for /docs and debugging.
# We construct the FastAPI application instance.
app = FastAPI(title="Code Analyser Model Service", version="1.1.0")


# Batch input and configurable threshold are sufficient for most uses.
...
class PredictRequest(BaseModel):
    ...
    # If true and a rationale model is available, the service will attach a single-sentence rationale per item.
    include_rationale: bool = Field(
        False, description="Attach a rationale string when supported (optional)."
    )


# Each item returns both activated labels and the raw per-label probabilities; rationale is optional.
...
class PredictItem(BaseModel):
    ...
    # Optional, short explanation either produced by a local seq2seq model or empty when not requested/unavailable.
    rationale: Optional[str] = None


# The response wraps results and a canonical label order for downstream stability.
...
# Request schema for the /explain endpoint used to generate a one-sentence rationale with an LLM/API.
class ExplainRequest(BaseModel):
    # The code snippet to explain (single item).
    code: str = Field(..., description="Single code snippet the user is inspecting.")
    # The top label key (e.g., 'sast_risk', 'ml_signal', 'best_practice') to steer the rationale.
    top_label: str = Field(..., description="Top label key to condition the rationale.")


# Response schema returned by /explain.
class ExplainResponse(BaseModel):
    # A concise, single-sentence explanation.
    rationale: str


# Resolve MODEL_DIR to an absolute path; default to models/trained_model under the repo.
...
# Read from environment, falling back to the default; then normalise to an absolute Path.
_model_env = os.environ.get("MODEL_DIR", str(_default_dir))
MODEL_DIR = Path(_model_env).expanduser().resolve()
# Fail fast if the directory does not exist to avoid accidental remote downloads.
if not MODEL_DIR.is_dir():
    raise FileNotFoundError(f"MODEL_DIR does not exist locally: {MODEL_DIR}")

# Optional local seq2seq rationale generator directory; when set and present, we will load it.
...
# Choose CUDA if available, otherwise CPU; the model is loaded once at startup.
# We pick the most capable device available (CUDA if present, else CPU).
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

# Force local file loading so we never hit Hugging Face by mistake.
...
model.to(device)
# Evaluation mode disables dropout for deterministic inference.
# We switch to evaluation mode to disable dropout etc.
model.eval()

# Load an optional local rationale model (seq2seq) if configured; otherwise keep None and rely on /explain if desired.
...
# Cache label order once for predictable indexing in the UI.
...
if _thresh_path.exists():
    try:
        ...
        PER_LABEL_THRESH = {}
# Prepare optional rationale generator components.
rationale_tokenizer = None
rationale_model = None
# Only attempt to load a local rationale model if a directory was provided and exists.
if RATIONALE_MODEL_DIR and RATIONALE_MODEL_DIR.is_dir():
    # Load a small seq2seq model (e.g., T5) for one-sentence rationales; strictly local.
    rationale_tokenizer = AutoTokenizer.from_pretrained(
        RATIONALE_MODEL_DIR.as_posix(), local_files_only=True
    )
    # Use a generic seq2seq head capable of conditional generation.
    rationale_model = AutoModelForSeq2SeqLM.from_pretrained(
        RATIONALE_MODEL_DIR.as_posix(), local_files_only=True
    )
    # Place rationale model on the same device for efficiency.
    rationale_model.to(device)
    # Ensure eval mode.
    rationale_model.eval()


# Stateless helper to classify a batch with sigmoid + threshold per label.
...
def classify_batch(texts: List[str], threshold: float) -> List[PredictItem]:
    # Truncate/pad to model max length (commonly 512 for RoBERTa).
...
# Generate a single-sentence rationale using a local seq2seq model when available.
def _local_rationale(code: str, top_label: str) -> Optional[str]:
    # If no local model is configured, indicate absence.
    if not (rationale_model and rationale_tokenizer):
        return None
    # Build a compact prompt that conditions on the top label and asks for one sentence.
    prompt = f"Explain in one concise British English sentence why the following code may be categorised as '{top_label}':\n{code}\nReason:"
    # Tokenise and generate with modest limits to keep latency low.
    enc = rationale_tokenizer(prompt, return_tensors="pt", truncation=True, max_length=512).to(
        device
    )
    # Use deterministic decoding with a small beam for quality.
    with torch.no_grad():
        out = rationale_model.generate(**enc, max_length=64, num_beams=4, early_stopping=True)
    # Decode and normalise whitespace.
    return rationale_tokenizer.decode(out[0], skip_special_tokens=True).strip()


# Fallback to an external LLM provider via HTTP APIs (OpenAI or Together) based on environment configuration.
def _external_rationale(code: str, top_label: str) -> Optional[str]:
    ...
    if not provider:
        return None
    # Normalise a minimal prompt for the API.
    prompt = f"One-sentence reason (British English): why is this code categorised as '{top_label}'?\n\n{code}\n\nReturn only one sentence."
    # OpenAI path expects OPENAI_API_KEY and model name (default: gpt-4o-mini).
    if provider == "openai":
        api_key = os.environ.get("OPENAI_API_KEY", "")
        model = os.environ.get("OPENAI_MODEL", "gpt-4o-mini")
        if not api_key:
            return None
        # Use the Chat Completions HTTP API directly to avoid extra dependencies.
    ...
    # Together path expects TOGETHER_API_KEY and model name (default: meta-llama/Meta-Llama-3-8B-Instruct-Turbo).
    if provider == "together":
        ...
        data = r.json()
        return data["choices"][0]["message"]["content"].strip()
    # Unknown provider; return None to indicate no rationale.
    return None


# Health endpoint assists readiness checks and quick model introspection.
# A simple health endpoint so orchestrators can probe readiness.
@app.get("/healthz")
def healthz() -> Dict[str, Any]:
    # Device and labels help confirm you are talking to the expected model.
...
# Main prediction endpoint used by the Code Analyser; can optionally attach a rationale per item if a local generator is set and requested.
# The main prediction endpoint used by the Code Analyser pipeline.
@app.post("/predict", response_model=PredictResponse)
def predict(req: PredictRequest) -> PredictResponse:
    # Run the batch classification with the caller-supplied threshold.
    # We run the batch classification with the caller-supplied threshold.
    results = classify_batch(req.texts, req.threshold)
    # If rationale was requested and a local generator is available, attach a single-sentence explanation for each item using its strongest label.
    ...
    # Return predictions and canonical order.
    # We return both results and the canonical label order.
    return PredictResponse(results=results, label_order=label_order)


# Secondary endpoint to obtain a one-sentence rationale from an LLM or local seq2seq; used by the GUI when richer explanations are desired.
@app.post("/explain", response_model=ExplainResponse)
def explain(req: ExplainRequest) -> ExplainResponse:
    # Try local generator first for privacy/latency.
    reason = _local_rationale(req.code, req.top_label)
    # Fall back to external provider if local generation is unavailable.
    if not reason:
        reason = _external_rationale(req.code, req.top_label)
    # Final fallback to a safe default if nothing could be generated.
    if not reason:
        reason = ""
    # Wrap and return the rationale.
    return ExplainResponse(rationale=reason)


# Allow running as a script for local testing; otherwise start via `python -m uvicorn ...`.
...
if __name__ == "__main__":
    ...
    # Start the server on 127.0.0.1:8111; adjust as needed.
