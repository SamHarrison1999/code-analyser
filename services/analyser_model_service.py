...
device = torch.device("cuda" if getattr(torch, "cuda", None) and torch.cuda.is_available() else "cpu")
...
id2label: Dict[int, str] = {int(i): l for i, l in (id2label_raw.items() if hasattr(id2label_raw, "items") else {"0":"LABEL_0","1":"LABEL_1"}.items())}
