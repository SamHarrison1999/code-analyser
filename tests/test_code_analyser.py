...
def install_transformers_stub():
    # Annotation: Remove any real or previously mocked 'transformers' modules so this stub is authoritative.
    for key in list(sys.modules.keys()):
        if key == "transformers" or key.startswith("transformers."):
            sys.modules.pop(key, None)
    # Annotation: Create a fresh 'transformers' package namespace.
...
def install_torch_stub():
    ...
    for key in list(sys.modules.keys()):
        if key == "torch" or key.startswith("torch."):
            sys.modules.pop(key, None)
    # Annotation: Construct a minimal but sufficient 'torch' package that satisfies training/inference code paths and test usage.
