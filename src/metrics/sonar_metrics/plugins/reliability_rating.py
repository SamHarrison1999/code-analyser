# File: code_analyser/src/metrics/sonar_metrics/plugins/reliability_rating.py
...
class ReliabilityRatingPlugin(SonarMetricPlugin):
    """
    Plugin to extract the 'reliability_rating' metric from SonarQube analysis data.
    """

    # ✅ Plugin metadata for plugin discovery and classification
    plugin_name = "reliability_rating"
    plugin_tags = ["reliability", "rating", "quality", "sonarqube"]
    ...
    def extract(self, sonar_data: dict[str, Any], file_path: str) -> float:
        ...
        """
        try:
            value = sonar_data.get("reliability_rating", 0)
            return float(value)
        except (ValueError, TypeError) as e:
            logging.warning(
                f"[ReliabilityRatingPlugin] Failed to extract 'reliability_rating' for {file_path}: {e}"
            )
            return 0.0
    ...
    def confidence_score(self, sonar_data: dict[str, Any]) -> float:
        ...
        """
        return 1.0 if "reliability_rating" in sonar_data else 0.0

    def severity_level(self, sonar_data: dict[str, Any]) -> str:
        """
        Classifies severity based on numeric reliability rating (1 = best, 5 = worst).

        Returns:
            str: One of 'low', 'medium', or 'high'.
        """
        try:
            rating = float(sonar_data.get("reliability_rating", 0))
            if rating <= 1.0:
                return "low"
            elif rating <= 3.0:
                return "medium"
