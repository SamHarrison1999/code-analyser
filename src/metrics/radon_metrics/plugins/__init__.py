# File: code_analyser/src/metrics/radon_metrics/plugins/__init__.py

"""
Radon Metric Plugin Loader

Dynamically discovers and loads RadonMetricPlugin subclasses from this directory.
...
# 🧠 ML Signal: Registry defines complexity metrics for supervised learning
    ...
    try:
        ...
        for name, obj in inspect.getmembers(module, inspect.isclass):
            if issubclass(obj, BasePlugin) and obj is not BasePlugin:
                ...
                logger.debug(f"✅ Loaded Radon plugin: {plugin_id} (tags={plugin_tags})")
    except Exception as e:
        logger.warning(
            f"⚠️ Failed to load Radon plugin module '{module_name}': {type(e).__name__}: {e}"
        )
...
def load_plugins() -> list[BasePlugin]:
    """Instantiate and return all discovered Radon metric plugins."""
    return [entry["class"]() for entry in _discovered_plugins.values()]


def load_plugins_by_tag(tag: str) -> list[BasePlugin]:
    """Return only Radon plugins tagged with a specific label."""
...
def list_plugins_metadata() -> list[dict]:
    """Return metadata about all registered Radon plugins."""
    return [
