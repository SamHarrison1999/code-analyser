...
def gated_fuse_one(
    ...
    rule_hits: set[str],
    th_model: dict | None = None,
    th_rule: dict | None = None,
) -> dict[str, bool]:
...
def gated_fuse_batch(
    scores_list: list[dict[str, float]],
    rule_hits_list: list[set[str]],
    th_model: dict | None = None,
    th_rule: dict | None = None,
) -> list[dict[str, bool]]:
    # Annotation: Zip scores and rule hits per example and fuse each with the same thresholds.
    return [
        gated_fuse_one(sc, rh, th_model=th_model, th_rule=th_rule)
        for sc, rh in zip(scores_list, rule_hits_list)
    ]
