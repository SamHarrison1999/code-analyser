...
class MultiLabelCollator:
    ...
    def __call__(self, features):
        ...
        batch = self.tokenizer.pad(feats, return_tensors="pt", pad_to_multiple_of=self.pad_to_multiple_of)
...
def build_training_args(**kw) -> TrainingArguments:
    ...
    try:
        _ = TrainingArguments(output_dir="tmp_probe", evaluation_strategy="epoch", save_strategy="epoch")
        extra = dict(
...
def main():
    ...
    targs1 = build_training_args(output_dir=head_out, lr=args.head_lr, epochs=max(1, args.head_epochs), bsz=args.bsz, warmup=args.warmup_ratio, seed=args.seed)
    ...
    targs2 = build_training_args(output_dir=full_out, lr=args.lr, epochs=remaining_epochs, bsz=args.bsz, warmup=args.warmup_ratio, seed=args.seed)
