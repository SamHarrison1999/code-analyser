# -*- coding:utf-8 -*-
# 🧠 ML Signal: URL patterns for financial data access

DP_URL = "%sapp.finance.%s/data/stock/%s?day=&page=%s"
# 🧠 ML Signal: URL patterns for financial data access
DP_163_URL = (
    "%squotes.%s/data/caibao/%s?reportdate=%s&sort=declaredate&order=desc&page=%s"
)
...
# 🧠 ML Signal: URL patterns for financial data access
XSG_URL = (
    "%sdatainterface.%s/EM_DataCenter/%s?type=FD&sty=BST&st=3&sr=true&fd=%s&stat=%s"
)
...
# 🧠 ML Signal: Column names for data processing
...
# 🧠 ML Signal: Categories for data processing
MAR_SZ_MX_COLS = [
    "stockCode",
    "securityAbbr",
    "rzmre",
...
MAR_SZ_HZ_MSG = (
    "please do not input more than a year,you can obtaining the data year by year."
)
# 🧠 ML Signal: Column names for data processing
MAR_SZ_HZ_MSG2 = "start and end date all need input."
# 🧠 ML Signal: Column names for data processing
# 🧠 ML Signal: Dictionary for date range mapping
# ⚠️ SAST Risk (Low): Hardcoded cookie string, potential security risk if sensitive
# ✅ Best Practice: User-friendly error message for input validation
# 🧠 ML Signal: URL patterns for financial data access
