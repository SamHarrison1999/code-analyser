...
def get_industry_classified(standard="sina"):
    ...
    if standard == "sw":
        ...
        df = pd.read_csv(
            ct.TSDATA_CLASS % (ct.P_TYPE["http"], ct.DOMAINS["oss"], "industry_sw"),
            dtype={"code": object},
        )
    else:
        #         df = _get_type_data(ct.SINA_INDUSTRY_INDEX_URL%(ct.P_TYPE['http'],
        ...
        df = pd.read_csv(
            ct.TSDATA_CLASS % (ct.P_TYPE["http"], ct.DOMAINS["oss"], "industry"),
            dtype={"code": object},
        )
    #     data = []
    #     ct._write_head()
...
def get_concept_classified():
    ...
    """
    df = pd.read_csv(
        ct.TSDATA_CLASS % (ct.P_TYPE["http"], ct.DOMAINS["oss"], "concept"),
        dtype={"code": object},
    )
    return df
...
def get_concepts(src="dfcf"):
    ...
    clt = Client(
        ct.ET_CONCEPTS_INDEX_URL % (ct.P_TYPE["http"], ct.DOMAINS["dfcf"], _random(15)),
        ref="",
    )
...
def _get_detail(tag, retry_count=3, pause=0.001):
    ...
    while True:
        ...
        for _ in range(retry_count):
            ...
            try:
                ...
                request = Request(
                    ...
                    % (ct.P_TYPE["http"], ct.DOMAINS["vsf"], ct.PAGES["jv"], p, tag)
                )
        ...
        js = json.loads(jstr)
        df = pd.DataFrame(
            pd.read_json(js, dtype={"code": object}), columns=ct.THE_FIELDS
        )
