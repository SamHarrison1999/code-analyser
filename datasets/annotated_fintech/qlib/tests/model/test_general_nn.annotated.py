...
class TestNN(TestAutoData):
    def test_both_dataset(self):
        ...
        model_l = [
            ...
            GeneralPTNN(
                n_epochs=2,
                # 🧠 ML Signal: Iterating over datasets and models to fit and predict
                batch_size=32,
                n_jobs=0,
                # 🧠 ML Signal: Model prediction on dataset
                pt_model_uri="qlib.contrib.model.pytorch_nn.Net",  # it is a MLP
                # ✅ Best Practice: Use of unittest.main() for running tests
                pt_model_kwargs={
                    "input_dim": 3,
                },
            ),
