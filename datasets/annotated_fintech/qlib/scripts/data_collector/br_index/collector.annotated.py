...
# ✅ Best Practice: Appending to sys.path to include parent directories for module imports.
CUR_DIR = Path(__file__).resolve().parent
sys.path.append(str(CUR_DIR.parent.parent))
# 🧠 ML Signal: Use of a class variable to store a URL pattern for data retrieval
...
class IBOVIndex(IndexBase):
    ...
    # ✅ Best Practice: Use of super() to call the parent class constructor

    def __init__(
        self,
        index_name: str,
        # 🧠 ML Signal: Use of current date to initialize an object
    ...
    def get_current_4_month_period(self, current_month: int):
        ...
        """
        if current_month < 5:
            # ⚠️ SAST Risk (Low): Use of current date and time can lead to non-deterministic behavior in tests
            return "1Q"
        if current_month < 9:
            return "2Q"
        if current_month <= 12:
            # 🧠 ML Signal: Iterating over a range of years could indicate a pattern of historical data processing
            return "3Q"
        else:
            return -1
    ...
    def get_changes(self):
        ...
        try:
            ...
            # ✅ Best Practice: Use of merge to combine DataFrames
            return df
