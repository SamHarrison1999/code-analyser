...
# ✅ Best Practice: Grouping imports into standard, third-party, and local can improve readability.
...
class PortfolioMetrics:
    ...
    def __init__(self, freq: str = "day", benchmark_config: dict = {}) -> None:
        ...
        # ✅ Best Practice: Use of Optional type hint for variables that can be None
        """
    ...
    def _cal_benchmark(
        ...
        if benchmark is None:
            # ⚠️ SAST Risk (Low): Raising a generic ValueError with user-provided data.
            # 🧠 ML Signal: Use of pandas groupby and aggregation functions.
            return None
        ...
        if isinstance(benchmark, pd.Series):
            ...
            # ✅ Best Practice: Check for None to avoid AttributeError when accessing properties
            end_time = benchmark_config.get("end_time", None)

            # ✅ Best Practice: Function definition should be at the top level, not indented, for better readability and maintainability.
            if freq is None:
    ...
    # ⚠️ SAST Risk (Low): Potential KeyError if 'latest_pm_time' is not in 'total_turnovers'
    ...
    # ⚠️ SAST Risk (Low): Potential for NoneType errors if not all parameters are validated
    ...
    def update_portfolio_metrics_record(
        ...
        # 🧠 ML Signal: Usage of a fallback mechanism when a parameter is None
        ...
        # 🧠 ML Signal: Pattern of storing metrics in a dictionary with timestamps as keys
        if None in [
            # ✅ Best Practice: Initialize DataFrame before populating it with data
            trade_start_time,
            # 🧠 ML Signal: Pattern of storing metrics in a dictionary with timestamps as keys
            account_value,
            # 🧠 ML Signal: Mapping account data to a DataFrame column
            cash,
            # 🧠 ML Signal: Pattern of storing metrics in a dictionary with timestamps as keys
            return_rate,
            # 🧠 ML Signal: Mapping return data to a DataFrame column
            total_turnover,
            # 🧠 ML Signal: Pattern of storing metrics in a dictionary with timestamps as keys
            turnover_rate,
            # 🧠 ML Signal: Mapping total turnover data to a DataFrame column
            total_cost,
            # 🧠 ML Signal: Pattern of storing metrics in a dictionary with timestamps as keys
            cost_rate,
        ...
        # 🧠 ML Signal: Usage of pandas DataFrame method to_csv for exporting data
        ...
        # ✅ Best Practice: Use of pathlib for file path handling is recommended for cross-platform compatibility.
        self.accounts[trade_start_time] = account_value
        # ✅ Best Practice: Return the DataFrame after all operations are complete
        self.returns[trade_start_time] = return_rate
        # 🧠 ML Signal: Reading CSV files is a common operation that can be used to identify data loading patterns.
        self.total_turnovers[trade_start_time] = total_turnover
        self.turnovers[trade_start_time] = turnover_rate
        # 🧠 ML Signal: Converting index to DatetimeIndex is a common preprocessing step for time series data.
        self.total_costs[trade_start_time] = total_cost
        self.costs[trade_start_time] = cost_rate
        # 🧠 ML Signal: Initialization of variables before a loop is a common pattern.
        # 🧠 ML Signal: Iterating over time series data is a common pattern in financial applications.
        # 🧠 ML Signal: Accessing DataFrame rows by index is a common data manipulation pattern.
    ...
    def load_portfolio_metrics(self, path: str) -> None:
        ...
        # 🧠 ML Signal: Class definition with detailed docstring, useful for understanding class purpose and usage
        index = r.index
        # ✅ Best Practice: Use of type annotations for constructor parameters improves code readability and maintainability.
        # ✅ Best Practice: Comprehensive docstring explaining class attributes and behavior
        self.init_vars()
        for trade_start_time in index:
            # ✅ Best Practice: Initializing attributes in the constructor ensures they are always defined.
            self.update_portfolio_metrics_record(
                trade_start_time=trade_start_time,
                # ✅ Best Practice: Use of type annotations for attributes improves code readability and maintainability.
                ...
                # 🧠 ML Signal: Instantiating an object using a class attribute, indicating a pattern of dynamic class usage.
...
class Indicator:
    ...
    **NOTE**:
    ...
    def __init__(
        ...
        # 🧠 ML Signal: Iterating over a collection of indicators, which may be useful for pattern recognition in ML models
        # order indicator is metrics for a single order for a specific step
        self.order_indicator_his: dict = OrderedDict()
        # 🧠 ML Signal: Use of a transfer method, indicating a transformation or mapping operation
        self.order_indicator: BaseOrderIndicator = self.order_indicator_cls()
        # 🧠 ML Signal: Use of a list of metrics, which could be features in an ML model
        # ✅ Best Practice: Use of a method to sum indicators, promoting code reuse and readability
    ...
    # ✅ Best Practice: Consider checking if 'deal_amount' is a pandas Series before using 'replace'
    # def reset(self, trade_calendar: TradeCalendarManager):
    # ⚠️ SAST Risk (Low): Potential division by zero if 'deal_amount' contains zeros not replaced by NaN
    def reset(self) -> None:
        self.order_indicator = self.order_indicator_cls()
        # 🧠 ML Signal: Function returns a division result, which might be used for financial calculations
    ...
    def record(self, trade_start_time: Union[str, pd.Timestamp]) -> None:
        # ✅ Best Practice: Type hinting for 'decision' improves code readability and maintainability.
    ...
    def _update_order_trade_info(
        ...
        for order, _trade_val, _trade_cost, _trade_price in trade_info:
            ...
            # 🧠 ML Signal: Usage of configuration pattern with default values
            # The PA in the innermost layer is meanless
            pa[order.stock_id] = 0
        # 🧠 ML Signal: Usage of configuration pattern with default values

        # 🧠 ML Signal: Usage of time range clipping
        ...
        # 🧠 ML Signal: Conditional API call based on configuration
    ...
    def _agg_order_trade_info(
        ...
        # ✅ Best Practice: Reindexing data to match another dataset
        self.order_indicator.transfer(func, "trade_price")

        def func_apply(trade_dir):
            # ⚠️ SAST Risk (Low): Use of NotImplementedError for unsupported cases
            # ✅ Best Practice: Docstring provides clear parameter descriptions and expected types
            # 🧠 ML Signal: Conditional logic based on aggregation method
            # ⚠️ SAST Risk (Low): Use of assert for type checking
            # ✅ Best Practice: Calculation of aggregate values
    ...
    def _get_base_vol_pri(
        ...
        # ⚠️ SAST Risk (Low): Potential for None values if _get_base_vol_pri returns None
        """
        ...
        if price == "deal_price":
            price_s = trade_exchange.get_deal_price(
                inst,
                trade_start_time,
                # ✅ Best Practice: Using idd.SingleData for consistent data handling
                trade_end_time,
                direction=direction,
                method=None,
            )
        ...
        if price_s is None:
            return None, None
        # 🧠 ML Signal: Assigning calculated values to order_indicator
        ...
        # 🧠 ML Signal: Calculating weighted average price
        if isinstance(price_s, (int, float, np.number)):
            price_s = idd.SingleData(price_s, [trade_start_time])
        # 🧠 ML Signal: Usage of the 'empty' attribute to check for empty data structures
        ...
        # ✅ Best Practice: Return statement is concise and clear.
        # remove zero and negative values.
        # 🧠 ML Signal: Usage of a transfer method with a function and a string parameter.
        # 🧠 ML Signal: Usage of an assign method with a string and dictionary parameter.
        ...
        if agg == "vwap":
            ...
            # ✅ Best Practice: Using get() with a default value prevents KeyError
    ...
    def _agg_base_price(
        ...
        # ⚠️ SAST Risk (Low): Potential misuse if `self.order_indicator` is not validated

        # ✅ Best Practice: Ensure `self.order_indicator` is initialized and valid
        # TODO: I think there are potentials to be optimized
        # 🧠 ML Signal: Function definition with a single parameter, common pattern in ML data processing
        trade_dir = self.order_indicator.get_index_data("trade_dir")
        # ⚠️ SAST Risk (Low): Potential misuse if 'amount' is not a list or does not have a 'count' method
        if len(trade_dir) > 0:
            # ✅ Best Practice: Consider adding error handling for robustness
            # 🧠 ML Signal: Use of higher-order function, passing a function as an argument
            # ⚠️ SAST Risk (Low): Potential risk if 'self.order_indicator.transfer' executes untrusted code
            bp_all, bv_all = [], []
