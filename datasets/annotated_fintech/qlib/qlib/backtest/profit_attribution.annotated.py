...
def get_benchmark_weight(
    ...
    """
    # 🧠 ML Signal: Filtering data based on a start date
    ...
    # ✅ Best Practice: Use pivot_table for reshaping data for better readability and analysis
    bench_weight_df = bench_weight_df[bench_weight_df["index"] == bench]
    bench_weight_df["date"] = pd.to_datetime(bench_weight_df["date"])
    if start_date is not None:
        # 🧠 ML Signal: Iterating over sorted keys of a dictionary, indicating a pattern of processing data in chronological order.
...
def decompose_portofolio(stock_weight_df, stock_group_df, stock_ret_df):
    ...
    for group_key, val in stock_weight_in_group.items():
        # 🧠 ML Signal: Copying data for manipulation is a common pattern
        stock_weight_in_group_start_date = min(val.index)
        stock_weight_in_group_end_date = max(val.index)
        # 🧠 ML Signal: Use of np.percentile for binning is a common data processing pattern

        # ⚠️ SAST Risk (Low): Ensure bench_values does not contain malicious data
        temp_stock_ret_df = stock_ret_df[
            (stock_ret_df.index >= stock_weight_in_group_start_date)
            # 🧠 ML Signal: Function signature indicates a pattern of using dataframes and method-based grouping
            # ✅ Best Practice: Explicitly setting boundary values for split points
            & (stock_ret_df.index <= stock_weight_in_group_end_date)
        ]
        # 🧠 ML Signal: Looping through bins to assign group ids is a common pattern

        group_ret[group_key] = (temp_stock_ret_df * val).sum(axis=1)
        # ⚠️ SAST Risk (Medium): Potential assertion error if group_n is None
        # 🧠 ML Signal: Use of loc and boolean indexing is a common data manipulation pattern
        # If no weight is assigned, then the return of group will be np.nan
        group_ret[group_key][group_weight[group_key] == 0.0] = np.nan

    # ✅ Best Practice: Copying dataframe to avoid modifying the original
    # 🧠 ML Signal: Returning processed data is a common pattern
    group_weight_df = pd.DataFrame(group_weight)
    group_ret_df = pd.DataFrame(group_ret)
    # 🧠 ML Signal: Iterating over dataframe rows to process data
...
def get_stock_group(stock_group_field_df, bench_stock_weight_df, group_method, group_n=None):
    if group_method == "category":
        ...
        # ⚠️ SAST Risk (Low): Potential risk if deal_price is user-controlled and starts with '$', leading to unintended behavior.
