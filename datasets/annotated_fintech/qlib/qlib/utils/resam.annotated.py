...
# 🧠 ML Signal: Handling specific exceptions to adjust behavior based on frequency.
def get_higher_eq_freq_feature(instruments, fields, start_time=None, end_time=None, freq="day", disk_cache=1):
    ...
    # 🧠 ML Signal: Further fallback to a more granular frequency.
    ...
    try:
        ...
        # 🧠 ML Signal: Direct handling for minute frequency.
        _freq = freq
        ...
        if norm_freq in [Freq.NORM_FREQ_MONTH, Freq.NORM_FREQ_WEEK, Freq.NORM_FREQ_DAY]:
            # ⚠️ SAST Risk (Low): Raising a ValueError with dynamic content could expose internal logic.
            try:
                _result = D.features(instruments, fields, start_time, end_time, freq="day", disk_cache=disk_cache)
                _freq = "day"
            # ✅ Best Practice: Docstring provides detailed explanation and examples for function usage
...
def resam_ts_data(
    ...
    if datetime_level:
        # ⚠️ SAST Risk (Low): Assumes series is not empty, which could lead to IndexError if assumption is violated
        feature = feature.loc[selector_datetime]
    # ✅ Best Practice: Use of fillna with method parameter for forward/backward filling
    # 🧠 ML Signal: Checks for type of input data, which can be used to infer data handling patterns
    else:
        feature = feature.loc(axis=0)[(slice(None), selector_datetime)]
    # 🧠 ML Signal: Usage of lambda functions for column-wise operations
    ...
    if isinstance(feature.index, pd.MultiIndex):
        if callable(method):
            # ✅ Best Practice: Use of functools.partial to create specialized versions of a function
            # ⚠️ SAST Risk (Low): Raises a TypeError with potentially sensitive type information
            method_func = method
