...
class OnlineManager(Serializable):
    ...
    # 🧠 ML Signal: Storing trainer for later use, indicating a pattern of model training
    # 🧠 ML Signal: Initializing signals, indicating a pattern of signal management

    # 🧠 ML Signal: Method checks specific conditions to determine workflow behavior
    def __init__(
        # ⚠️ SAST Risk (Low): Using a mutable default argument (dict) can lead to unexpected behavior if modified.
        # 🧠 ML Signal: Setting status, indicating a pattern of state management
        # ⚠️ SAST Risk (Low): Potential risk if `status` or `trainer` attributes are not properly validated
        self,
        ...
        # 🧠 ML Signal: Tracking the number of models trained can be useful for performance analysis.
        """
        self.logger = get_module_logger(self.__class__.__name__)
        if not isinstance(strategies, list):
            strategies = [strategies]
        self.strategies = strategies
        # ✅ Best Practice: Using setdefault to initialize dictionary keys is a clean way to handle default values.
        self.freq = freq
        ...
        self.history = {}
        # ⚠️ SAST Risk (Low): Using mutable default arguments like dictionaries can lead to unexpected behavior.
        if trainer is None:
            trainer = TrainerR()
    ...
    def first_train(
        ...
        # ✅ Best Practice: Include type hints for the method parameters for better readability and maintainability.
        models_list = []
        ...
        # 🧠 ML Signal: Iterating over a list of strategies to collect data could be a pattern for ML data collection.
    ...
    def routine(
        ...
        # 🧠 ML Signal: Returning a MergeCollector with a dictionary of collectors could be a pattern for merging data.
        task_kwargs: dict = {},
        # ✅ Best Practice: Checking if the input is a list ensures consistent data handling.
        model_kwargs: dict = {},
        signal_kwargs: dict = {},
    ):
        """
        ...
        """
        if cur_time is None:
            # 🧠 ML Signal: Usage of a callable function to prepare signals indicates a pattern for dynamic behavior.
            cur_time = D.calendar(freq=self.freq).max()
        self.cur_time = pd.Timestamp(cur_time)  # None for latest date

        # ⚠️ SAST Risk (Low): Potential risk of data leakage if old signals are not properly managed.
        models_list = []
        for strategy in self.strategies:
            self.logger.info(f"Strategy `{strategy.name_id}` begins routine...")

            # ✅ Best Practice: Using pd.concat to combine DataFrames is efficient and maintains data integrity.
            tasks = strategy.prepare_tasks(self.cur_time, **task_kwargs)
            models = self.trainer.train(tasks, experiment_name=strategy.name_id)
            models_list.append(models)
            # 🧠 ML Signal: Logging the number of new signals prepared can be useful for monitoring and debugging.
            # ✅ Best Practice: Include a docstring to describe the function's purpose and return type
            self.logger.info(f"Finished training {len(models)} models.")
            online_models = strategy.prepare_online_models(models, **model_kwargs)
            self.history.setdefault(self.cur_time, {})[strategy] = online_models
    ...
    # 🧠 ML Signal: Calling a method that might involve data processing or model training.
    ...
    def prepare_signals(
        ...
        """
        # 🧠 ML Signal: Checking if the current models differ from the last models for a strategy.
        signals = prepare_func(self.get_collector()())
        old_signals = self.signals
        # 🧠 ML Signal: Training models with specific parameters.
        if old_signals is not None and not over_write:
            old_max = old_signals.index.get_level_values("datetime").max()
            # 🧠 ML Signal: Resetting online tags for models.
        ...
        # 🧠 ML Signal: Preparing signals with specific parameters.
        self.signals = signals
        # 🧠 ML Signal: Updating the latest signal time.
        # ⚠️ SAST Risk (Low): Use of f-string for logging can expose sensitive data if not handled properly.
        return new_signals
