...
# ✅ Best Practice: Class variables should be initialized with a default value or in the constructor.
...
class RecordTemp:
    ...
    # ✅ Best Practice: Check if the provided path is not None before appending
    artifact_path = None
    # ✅ Best Practice: Docstring provides a clear explanation of the method's purpose and usage.
    # ✅ Best Practice: Use of join to construct a path string from list elements
    ...
    def get_path(cls, path=None):
        # 🧠 ML Signal: Usage of self.get_path() indicates a pattern of retrieving paths, which can be useful for ML models to understand method interactions.
        names = []
        if cls.artifact_path is not None:
            names.append(cls.artifact_path)
        # ✅ Best Practice: Explicitly checking for an empty string improves code readability and intent.

        # 🧠 ML Signal: Use of constructor to initialize object state
        # 🧠 ML Signal: Usage of self.recorder.save_objects() shows a pattern of saving objects, which can be useful for ML models to understand method interactions.
        if path is not None:
            names.append(path)
        # 🧠 ML Signal: Use of private attribute naming convention
    ...
    def check(self, include_self: bool = False, parents: bool = True):
        ...
        """
        # 🧠 ML Signal: Use of dynamic function call with unpacked parameters
        if include_self:
            # Some mlflow backend will not list the directly recursively.
            # So we force to the directly
            # ✅ Best Practice: Handling specific exception to modify parameters
            artifacts = {}

            def _get_arts(dirn):
                if dirn not in artifacts:
                    # 🧠 ML Signal: Use of model prediction on a dataset
            ...
            for item in self.list():
                ...
                if self.get_path(item) not in _get_arts(dirn):
                    # ⚠️ SAST Risk (Low): Potential risk if 'save' method does not handle file overwriting securely
                    raise FileNotFoundError
        # ✅ Best Practice: Use of logging for tracking important events
        if parents:
            if self.depend_cls is not None:
                with class_casting(self, self.depend_cls):
...
# 🧠 ML Signal: Displaying the first few prediction results
# 🧠 ML Signal: Returning a list of model-related filenames could indicate a pattern of model file management
class SignalRecord(RecordTemp):
    ...
    def generate_label(dataset):
        with class_casting(dataset, DatasetH):
            ...
            try:
                ...
                # ⚠️ SAST Risk (Low): Catching broad exception without handling it properly
                # The argument number is not right
                del params["data_key"]
                # The backend handler should be DataHandler
                raw_label = dataset.prepare(**params)
            # 🧠 ML Signal: Checking if the result is a dictionary before saving
    ...
    def generate(self, **kwargs):
        ...
        # ✅ Best Practice: Class variables should be defined at the top of the class for better readability.
        ...
        # ⚠️ SAST Risk (Low): Ensure 'calc_ic' handles unexpected data types or values.
...
# ⚠️ SAST Risk (Low): Potential division by zero if 'ric.std()' is zero.
class ACRecordTemp(RecordTemp):
    """Automatically checking record template"""

    # ⚠️ SAST Risk (Low): Ensure 'calc_long_short_return' handles unexpected data types or values.
    ...
    def generate(self, *args, **kwargs):
        ...
        # ⚠️ SAST Risk (Low): Potential division by zero if 'long_short_r.std()' is zero.
        ...
        # 🧠 ML Signal: Method returning a list of filenames, indicating potential file handling or data processing
        try:
            self.check()
        # 🧠 ML Signal: Returning a hardcoded list of filenames, which may indicate specific data processing tasks
        except FileNotFoundError:
            # 🧠 ML Signal: Logging metrics can be used to track model performance over time.
            # 🧠 ML Signal: Saving objects can be used to track model outputs and states.
        ...
        if isinstance(artifact_dict, dict):
            # ✅ Best Practice: Class-level constants should be defined at the top of the class for better readability.
...
class HFSignalRecord(SignalRecord):
    ...
    # ✅ Best Practice: Loading default data when no input is provided.
    ...
    def generate(self):
        ...
        metrics = {
            "IC": ic.mean(),
            "ICIR": ic.mean() / ic.std(),
            # ✅ Best Practice: Use of a dictionary to store objects for better organization.
            "Rank IC": ric.mean(),
        ...
        # ✅ Best Practice: Logging metrics for tracking and analysis.
        self.save(**objects)
...
class SigAnaRecord(ACRecordTemp):
    ...
    # ✅ Best Practice: Explicitly defining dependencies improves code readability and maintainability.
    """
...
class PortAnaRecord(ACRecordTemp):
    ...
    depend_cls = SignalRecord
    # ✅ Best Practice: Ensuring indicator_analysis_freq is always a list

    def __init__(
        # 🧠 ML Signal: Parsing frequency strings into a specific format
        # ✅ Best Practice: Initialize variables at the start of the function for clarity.
        self,
        recorder,
        # 🧠 ML Signal: Checks for specific configuration keys, indicating feature usage patterns.
        config=None,
        risk_analysis_freq: Union[List, str] = None,
        # 🧠 ML Signal: Parsing frequency strings into a specific format
        # 🧠 ML Signal: Usage of Freq.parse suggests a pattern of frequency parsing.
        ...
        **kwargs,
        ...
        # ✅ Best Practice: Checking for None before assignment
        """
        super().__init__(recorder=recorder, skip_existing=skip_existing, **kwargs)

        # 🧠 ML Signal: Use of dictionary to store artifacts
        if config is None:
            config = {  # Default config for daily trading
                # 🧠 ML Signal: Function call with dynamic configuration
                "strategy": {
                ...
                "backtest": {
                    # 🧠 ML Signal: Dynamic key-value updates in dictionary
                    "start_time": None,
                    "end_time": None,
                    # 🧠 ML Signal: Iterating over dictionary items
                    "account": 100000000,
        ...
        # 🧠 ML Signal: Dictionary access and unpacking

        self.strategy_config = config["strategy"]
        # 🧠 ML Signal: Use of dictionary to store analysis results
        _default_executor_config = {
            "class": "SimulatorExecutor",
            # 🧠 ML Signal: Calculation of excess return
        ...
        # 🧠 ML Signal: Flattening dictionary for logging
        if risk_analysis_freq is None:
            risk_analysis_freq = [self.all_freq[0]]
        # 🧠 ML Signal: Logging metrics with dynamic keys
        ...
        # 🧠 ML Signal: Pretty printing analysis results
    ...
    def _get_report_freq(self, executor_config):
        # 🧠 ML Signal: Conditional check for presence in dictionary
        # 🧠 ML Signal: Conditional logic to append specific file paths based on frequency
        ret_freq = []
    ...
    def _generate(self, **kwargs):
        ...
        # 🧠 ML Signal: Dynamic key-value updates in dictionary
        # ⚠️ SAST Risk (Low): Use of warnings.warn without specifying a category
        # ✅ Best Practice: Return the constructed list at the end of the function
        # 🧠 ML Signal: Converting dataframe to dictionary
        ...
        # 🧠 ML Signal: Logging information with dynamic content
        # 🧠 ML Signal: Pretty printing analysis results
        # 🧠 ML Signal: Inheritance from PortAnaRecord suggests a pattern of extending functionality
        artifact_objects = {}
        # 🧠 ML Signal: Use of a class-level attribute to define dependencies
        # custom strategy and get backtest
        # 🧠 ML Signal: Returning a dictionary of artifacts
        ...
        for _analysis_freq in self.risk_analysis_freq:
            if _analysis_freq not in portfolio_metric_dict:
                warnings.warn(
                    # ✅ Best Practice: Using super() to call the parent class's __init__ method is a good practice for inheritance
                    f"the freq {_analysis_freq} report is not found, please set the corresponding env with `generate_portfolio_metrics=True`"
                )
            # ⚠️ SAST Risk (Low): deepcopy_basic_type might not handle all types correctly, ensure it's safe for the data
            else:
                report_normal, _ = portfolio_metric_dict.get(_analysis_freq)
                # ⚠️ SAST Risk (Low): Type checking with isinstance can be bypassed if the object is not of the expected type
                # 🧠 ML Signal: Loading data from a file, which is a common pattern in ML workflows
                analysis = dict()
        ...
        for _analysis_freq in self.indicator_analysis_freq:
            # 🧠 ML Signal: Adding metadata to a dataframe
    ...
    # 🧠 ML Signal: Flattening a dictionary for logging purposes
    def list(self):
        list_path = []
        # 🧠 ML Signal: Iterating over a class attribute
        for _freq in self.all_freq:
            list_path.extend(
                # 🧠 ML Signal: Conditional check against another class attribute
                [
