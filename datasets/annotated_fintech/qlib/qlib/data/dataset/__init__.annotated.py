...
class DatasetH(Dataset):
    ...
    def config(self, handler_kwargs: dict = None, **kwargs):
        ...
        # 🧠 ML Signal: Usage of dynamic keyword arguments with **kwargs
        """
        # 🧠 ML Signal: Usage of dynamic keyword arguments with **kwargs
    ...
    def _prepare_seg(self, slc, **kwargs):
        ...
        if hasattr(self, "fetch_kwargs"):
            # 🧠 ML Signal: Usage of isinstance to handle different types of 'segments'
    ...
    def prepare(
        self,
        # 🧠 ML Signal: Function usage pattern for calculating minimum time
        segments: Union[List[Text], Tuple[Text], Text, slice, pd.Index],
        col_set=DataHandler.CS_ALL,
        data_key=DataHandlerLP.DK_I,
        # 🧠 ML Signal: Function uses a lambda for comparison, indicating custom behavior
        **kwargs,
    ...
    @staticmethod
    # 🧠 ML Signal: Use of pandas MultiIndex, indicating hierarchical data structure, common in time-series analysis
    def get_max_time(segments):
        # 🧠 ML Signal: Use of numpy arrays for efficient data indexing and manipulation
        # 🧠 ML Signal: Use of pandas DataFrame for structured data storage and manipulation
...
class TSDataSampler:
    ...
    # 🧠 ML Signal: Slicing index map and data index based on start and end
    """

    # ⚠️ SAST Risk (Low): Use of assert for input validation can be disabled with optimization flags
    # Please refer to the docstring of TSDataSampler for the definition of following attributes
    data_arr: np.ndarray
    data_index: pd.MultiIndex
    # ✅ Best Practice: Use of np.array with dtype for consistent data type
    idx_map: np.ndarray
    # ✅ Best Practice: Consider adding error handling for potential exceptions in slice_locs
    # ✅ Best Practice: Deleting unused data to free memory
    idx_df: pd.DataFrame
    # ✅ Best Practice: Use descriptive variable names for better readability

    def __init__(
        self,
        # 🧠 ML Signal: Use of numpy data types and functions
        # 🧠 ML Signal: Returns a filtered subset of data, indicating a pattern of data slicing
        data: pd.DataFrame,
        start,
        # ✅ Best Practice: Use of numpy's iinfo to determine max value for dtype
        end,
        step_len: int,
        # 🧠 ML Signal: Use of dictionary keys to determine range
        ...
        # 🧠 ML Signal: Use of lazy_sort_index indicates a custom sorting mechanism
        self.data_arr = np.append(
            self.data_arr,
            # 🧠 ML Signal: Repeated use of lazy_sort_index suggests a pattern for data transformation
        ...
        # ✅ Best Practice: Method name 'empty' clearly indicates its purpose, improving readability.
        # The index of usable data is between start_idx and end_idx
        self.idx_df, self.idx_map = self.build_index(self.data)
        # ✅ Best Practice: Using 'len(self) == 0' is a clear and efficient way to check for emptiness.
        self.data_index = deepcopy(self.data.index)
        # ✅ Best Practice: Returning a tuple of DataFrame and dict for clear function output
    ...
    @staticmethod
    # 🧠 ML Signal: Use of forward fill method
    def slice_idx_map_and_data_index(
        idx_map,
        idx_df,
        # ✅ Best Practice: Docstring provides clear explanation of parameters and return values
        # 🧠 ML Signal: Use of forward and backward fill method
        # ⚠️ SAST Risk (Low): Use of assert statement for runtime check
        data_index,
        start,
        end,
    ):
    ...
    @staticmethod
    # ⚠️ SAST Risk (Low): Potential IndexError if self.idx_map is not properly validated
    def idx_map2arr(idx_map):
        ...
        # ⚠️ SAST Risk (Low): KeyError could expose internal state information
        # So we convert the dict into int array.
        # The arr_map is expected to behave the same as idx_map

        # 🧠 ML Signal: Use of pd.Timestamp indicates handling of date/time data
        dtype = np.int32
        # set a index out of bound to indicate the none existing
        no_existing_idx = (np.iinfo(dtype).max, np.iinfo(dtype).max)
        # 🧠 ML Signal: Use of bisect methods indicates sorted data handling

        max_idx = max(idx_map.keys())
        # ⚠️ SAST Risk (Low): NotImplementedError could expose internal state information
    ...
    def flt_idx_map(flt_data, idx_map):
        ...
        for i, exist in enumerate(flt_data):
            # 🧠 ML Signal: Use of isinstance to check for multiple types.
            if exist:
                # 🧠 ML Signal: Use of list comprehension for processing elements.
    ...
    def config(self, **kwargs):
        ...
        # 🧠 ML Signal: Usage of len() on a custom object indicates the object is likely a collection or has a collection-like behavior.
        for k, v in kwargs.items():
            # 🧠 ML Signal: Reshaping numpy arrays.
            setattr(self, k, v)
    ...
    def build_index(data: pd.DataFrame) -> Tuple[pd.DataFrame, dict]:
        ...
        # 🧠 ML Signal: Use of bisect to find insertion point in a sorted list.
        """
        # object incase of pandas converting int to float
        # ✅ Best Practice: Use of max to ensure index does not go out of bounds.
        idx_df = pd.Series(range(data.shape[0]), index=data.index, dtype=object)
        # ⚠️ SAST Risk (Low): Potential IndexError if cal is empty.
        idx_df = lazy_sort_index(idx_df.unstack())
        # NOTE: the correctness of `__getitem__` depends on columns sorted here
        idx_df = lazy_sort_index(idx_df, axis=1).T

        # ✅ Best Practice: Returning a slice object maintains consistency with input type.
        # 🧠 ML Signal: Use of kwargs for flexible function arguments
        idx_map = {}
        for i, (_, row) in enumerate(idx_df.iterrows()):
            for j, real_idx in enumerate(row):
                # ⚠️ SAST Risk (Low): Potential for unexpected behavior if slc is not a slice
                if not np.isnan(real_idx):
                    idx_map[real_idx] = (i, j)
        return idx_df, idx_map

    # ✅ Best Practice: Use of descriptive variable names like ext_slice
    @property
    def empty(self):
        # 🧠 ML Signal: Use of inheritance and method overriding
        return len(self) == 0

    # ✅ Best Practice: Use of deepcopy to avoid side effects
