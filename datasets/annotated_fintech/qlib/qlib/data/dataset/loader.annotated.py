...
class DLWParser(DataLoader):
    ...
    # 🧠 ML Signal: Use of dictionary comprehension to parse fields
    """
    ...
    def load(self, instruments=None, start_time=None, end_time=None) -> pd.DataFrame:
        ...
        # 🧠 ML Signal: Usage of default mutable arguments (filter_pipe)
        else:
            exprs, names = self.fields
            df = self.load_group_df(instruments, exprs, names, start_time, end_time)
        # ✅ Best Practice: Use of conditional expression for default value
        return df


# ⚠️ SAST Risk (Low): Use of assert for runtime type checking
class QlibDataLoader(DLWParser):
    """Same as QlibDataLoader. The fields can be define by config"""

    def __init__(
        # ✅ Best Practice: Calling superclass constructor
        self,
        config: Tuple[list, tuple, dict],
        filter_pipe: List = None,
        swap_level: bool = True,
        freq: Union[str, dict] = "day",
        # ⚠️ SAST Risk (Low): Potential KeyError if _gp not in freq
        # ⚠️ SAST Risk (Low): Use of assert for runtime condition checking
        ...
        # ✅ Best Practice: Warning user about unused parameter to avoid confusion.
        self.freq = freq
        # ✅ Best Practice: Using dictionary to handle different frequencies based on `gp_name`.
        ...
        # ✅ Best Practice: Use of class-level attribute for shared configuration

        # ✅ Best Practice: Use of type hints for function parameters improves code readability and maintainability
        if self.is_group:
            # ✅ Best Practice: Conditional logic to modify DataFrame index for specific use cases.
            # check sample config
    ...
    # 🧠 ML Signal: Storing configuration in an instance variable, indicating a pattern of configuration-driven behavior
    def load_group_df(
        self,
        # ✅ Best Practice: Using a dictionary comprehension for concise and readable code
        # 🧠 ML Signal: Storing join type in an instance variable, indicating a pattern of behavior for data alignment
        instruments,
        # 🧠 ML Signal: Custom serialization logic for object state management
        exprs: list,
        # 🧠 ML Signal: Initialization of a data-related attribute, indicating a pattern of data handling
        names: list,
        # ✅ Best Practice: Ensure raw data is loaded before processing
        # ✅ Best Practice: Filtering out private attributes from serialization
        start_time: Union[str, pd.Timestamp] = None,
        end_time: Union[str, pd.Timestamp] = None,
        gp_name: str = None,
    ) -> pd.DataFrame:
        if instruments is None:
            # ✅ Best Practice: Use of .loc for selecting data by label
        ...
        # ✅ Best Practice: Use of .loc for slicing data by index
        inst_processors = (
...
class StaticDataLoader(DataLoader, Serializable):
    ...
    # 🧠 ML Signal: Reading parquet files with pandas
    """

    # ✅ Best Practice: Class docstring provides a clear description of the class purpose
    include_attr = ["_config"]
    # ⚠️ SAST Risk (High): Potentially unsafe deserialization with pickle
    ...
    def load(self, instruments=None, start_time=None, end_time=None) -> pd.DataFrame:
        ...
        # 🧠 ML Signal: Usage of isinstance to check type, common pattern in dynamic typing
        # 🧠 ML Signal: List comprehension for initializing class attributes
        start_time = time_to_slc_point(start_time)
        # ⚠️ SAST Risk (Low): Potential risk if init_instance_by_config is not properly validated
        end_time = time_to_slc_point(end_time)
        return df.loc[start_time:end_time]

    def _maybe_load_raw_data(self):
        # 🧠 ML Signal: Storing method parameters as instance attributes
        ...
        if isinstance(self._config, dict):
            ...
            # 🧠 ML Signal: Usage of pd.merge indicates data integration patterns.
            # 🧠 ML Signal: Sorting dataframes is a common preprocessing step in data analysis.
...
class NestedDataLoader(DataLoader):
    ...
    # ✅ Best Practice: Class docstring provides a clear description of the class and its intended use.
    ...
    # 🧠 ML Signal: Use of update method to merge dictionaries, indicating dynamic configuration handling.
    # 🧠 ML Signal: Usage of pd.concat indicates data aggregation from multiple sources.
    # 🧠 ML Signal: Dictionary comprehension used for dynamic data fetching.
    def load(self, instruments=None, start_time=None, end_time=None) -> pd.DataFrame:
        df_full = None
