...
def risk_analysis(
    ...
    # ⚠️ SAST Risk (Low): N and freq are used without initial validation
    """

    def cal_risk_analysis_scaler(freq):
        # ⚠️ SAST Risk (Low): Warnings are not always handled by users
    ...
    if mode == "sum":
        ...
        # ⚠️ SAST Risk (Low): Potential for unsupported mode values
        # geometric mean (compound annual growth rate)
        # 🧠 ML Signal: Calculation of information ratio
        ...
        # ✅ Best Practice: Use of pd.Series for structured data representation
...
def backtest_daily(
    ...
    if executor is None:
        ...
        # ⚠️ SAST Risk (Low): Potential issue with level index check, ensure `get_level_index` is defined and used correctly.
    ...
    # ⚠️ SAST Risk (Low): Ensure `deal_price` is a string before checking its first character.
    if exchange_kwargs is not None:
        _exchange_kwargs.update(exchange_kwargs)

    # ⚠️ SAST Risk (Low): Using mutable default argument `subscribe_fields`, which can lead to unexpected behavior.
...
def long_short_backtest(
    ...
    if deal_price[0] != "$":
        # ✅ Best Practice: Import statements for required modules should be included at the top of the file.
        deal_price = "$" + deal_price

    subscribe_fields = subscribe_fields.copy()
    # 🧠 ML Signal: Reading a CSV file into a DataFrame is a common pattern in data processing tasks.
    profit_str = f"Ref({deal_price}, -1)/{deal_price} - 1"
    # ⚠️ SAST Risk (Low): File path is hardcoded, which may lead to issues if the file location changes.
    subscribe_fields.append(profit_str)

    # 🧠 ML Signal: Converting a column to datetime is a common preprocessing step in time series analysis.
    # 🧠 ML Signal: Setting a DataFrame index using multiple columns is a common pattern in data manipulation.
    trade_exchange = get_exchange(
        pred=pred,
        deal_price=deal_price,
        subscribe_fields=subscribe_fields,
        limit_threshold=limit_threshold,
        # 🧠 ML Signal: Slicing a DataFrame to limit the number of rows is a common data preparation step.
        open_cost=open_cost,
