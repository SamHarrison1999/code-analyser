...
# ⚠️ SAST Risk (Medium): Relative import can lead to import errors if the module structure changes
...
def _calculate_maximum(df: pd.DataFrame, is_ex: bool = False):
    ...
    """
    if is_ex:
        # ⚠️ SAST Risk (Low): Potential KeyError if expected columns are missing from the DataFrame.
...
# 🧠 ML Signal: Cumulative sum operations on DataFrame columns, common in financial data analysis
def _calculate_report_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # ✅ Best Practice: Consider specifying the return type using a type hint for better readability and maintainability
    report_df = pd.DataFrame()
    # 🧠 ML Signal: Direct assignment of DataFrame columns, indicating data transformation
    ...
    # ✅ Best Practice: Sorting the DataFrame by index to maintain order, improving data consistency
    ...
    # ⚠️ SAST Risk (Low): Directly modifying DataFrame index can lead to unexpected behavior
