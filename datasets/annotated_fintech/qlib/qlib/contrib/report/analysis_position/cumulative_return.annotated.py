...
# 🧠 ML Signal: Function signature with multiple parameters, including optional ones
import pandas as pd
...
def _get_cum_return_data_with_position(
    ...
    _cumulative_return_df = get_position_data(
        # ✅ Best Practice: Clear and concise DataFrame column operations
        position=position,
        report_normal=report_normal,
        # ✅ Best Practice: Use of .dropna() to handle missing data
    ...
    for gp in df_gp:
        ...
        # ✅ Best Practice: Conditional expression for division to avoid ZeroDivisionError
        ...
        result_list.append(
            dict(
                # ✅ Best Practice: Use of a dictionary to store related data
                ...
                # 🧠 ML Signal: Conversion of list of dictionaries to DataFrame
    ...
    # ✅ Best Practice: Consider adding type hints for start_date and end_date for better clarity.
    return r_df


def _get_figure_with_position(
    # 🧠 ML Signal: Iterating over a fixed list of strings to generate subplots.
    position: dict,
    report_normal: pd.DataFrame,
    label_data: pd.DataFrame,
    start_date=None,
    ...
    for _t_name in ["buy", "sell", "buy_minus_sell", "hold"]:
        ...
        layout = dict(
            ...
            shapes=[
                {
                    # ⚠️ SAST Risk (Low): Potential risk if SubplotsGraph is not properly validated or sanitized.
                    "type": "line",
                    "xref": "x2",
                    "yref": "paper",
                    "x0": mean_value,
