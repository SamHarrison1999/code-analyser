...
# ✅ Best Practice: Alias for better readability and to avoid namespace conflicts
...
# 🧠 ML Signal: Storing hyperparameter ranges for model optimization
...
class HyperparamOptManager:
    ...
    def __init__(
        ...
        """
        # 🧠 ML Signal: Loading data from CSV files into DataFrames

        self.param_ranges = param_ranges
        # 🧠 ML Signal: Checking if DataFrame is empty

        self._max_tries = 1000
        # 🧠 ML Signal: Applying a function to a DataFrame column
        self.results = pd.DataFrame()
        # 🧠 ML Signal: Method for retrieving parameters by key, useful for model configuration
        self.fixed_params = fixed_params
        # 🧠 ML Signal: Finding the minimum value in a DataFrame column
        self.saved_params = pd.DataFrame()
        # ⚠️ SAST Risk (Low): Potential KeyError if 'name' is not in 'params'

        # 🧠 ML Signal: Boolean indexing in DataFrames
        self.best_score = np.Inf
        # 🧠 ML Signal: Conditional logic for overriding parameters, indicating dynamic configuration
        self.optimal_name = ""
        # 🧠 ML Signal: Accessing DataFrame index

        # 🧠 ML Signal: Iterating over fixed parameters, indicating a pattern of parameter management
        ...
        # ✅ Best Practice: Explicitly setting dictionary values for clarity
        self.hyperparam_folder = model_folder
        # 🧠 ML Signal: Accessing a property that likely stores the name of the optimal model or configuration
        utils.create_folder_if_not_exist(self.hyperparam_folder)
        # ⚠️ SAST Risk (High): Use of shutil.rmtree can delete important files if hyperparam_folder is not validated
        # ✅ Best Practice: Returning a dictionary copy to prevent external modifications

        # 🧠 ML Signal: Method call to retrieve parameters based on a model or configuration name
        # ✅ Best Practice: Consider adding error handling for file operations
        self._override_w_fixed_params = override_w_fixed_params

    # ✅ Best Practice: Ensure the directory is created after clearing
    def load_results(self):
        """Loads results from previous hyperparameter optimisation.

        Returns:
          A boolean indicating if previous results can be loaded.
        # 🧠 ML Signal: Use of parameter validation logic
        """
        print("Loading results from", self.hyperparam_folder)
        # 🧠 ML Signal: List comprehension for filtering invalid fields

        results_file = os.path.join(self.hyperparam_folder, "results.csv")
        # 🧠 ML Signal: List comprehension for filtering missing fields
        ...
        if os.path.exists(results_file) and os.path.exists(params_file):
            ...
            # ✅ Best Practice: Ensure params is a dictionary before processing

            # ⚠️ SAST Risk (Low): Potential information disclosure in error message
            if not self.results.empty:
                # ✅ Best Practice: Use list comprehension for concise and readable code
                self.results.at["loss"] = self.results.loc["loss"].apply(float)
                self.best_score = self.results.loc["loss"].min()
                # ✅ Best Practice: Sorting ensures consistent order for generating unique keys

                # 🧠 ML Signal: Pattern of generating unique keys from dictionary parameters
    ...
    def _get_params_from_name(self, name):
        ...
        # ⚠️ SAST Risk (Low): Lack of type checking for self.param_ranges could lead to unexpected errors if it's not a dictionary.
        params = self.saved_params

        # ✅ Best Practice: Sorting keys ensures consistent order, which can prevent non-deterministic behavior.
        selected_params = dict(params[name])
        # 🧠 ML Signal: Random choice of hyperparameters indicates a search strategy for model optimization
    ...
    def get_best_params(self):
        # 🧠 ML Signal: Naming convention for parameter sets could be used to track experiments
    ...
    def _check_params(self, params):
        ...
        # ⚠️ SAST Risk (Low): Directory creation without exception handling
        ...
        # 🧠 ML Signal: Usage of hyperparameters to generate a unique name
    ...
    def _get_name(self, params):
        # ⚠️ SAST Risk (Medium): Potential risk if model.save() does not handle exceptions
        """Returns a unique key for the supplied set of params."""

        # 🧠 ML Signal: Updating the best score and optimal model name
        self._check_params(params)

        fields = list(params.keys())
        # 🧠 ML Signal: Storing results and parameters for each model
        # ⚠️ SAST Risk (Low): File operations without exception handling
        fields.sort()
    ...
    def get_next_parameters(self, ranges_to_skip=None):
        ...
        for _ in range(self._max_tries):
            ...
            # ⚠️ SAST Risk (Low): Potential for ValueError if worker_number is greater than search_iterations.
    ...
    def update_score(self, parameters, loss, model, info=""):
        ...
        """
        # 🧠 ML Signal: Option to clear serialized parameters indicates flexibility in hyperparameter management.

        if np.isnan(loss):
            # 🧠 ML Signal: Logging regeneration of hyperparameter list can be useful for tracking and debugging.
            loss = np.Inf
        # ⚠️ SAST Risk (Low): Potential risk if os.path.exists and shutil.rmtree are not atomic.

        if not os.path.isdir(self.hyperparam_folder):
            # ✅ Best Practice: Method name should follow PEP 8 naming conventions, consider renaming to `is_optimisation_completed`.
            os.makedirs(self.hyperparam_folder)

        # ✅ Best Practice: Use of utility function to create folder if not exists improves code reuse and readability.
        # ✅ Best Practice: Simplify return statement to `return not self.worker_search_queue`.
        name = self._get_name(parameters)

        # 🧠 ML Signal: Usage of hyperparameters suggests this is part of a machine learning workflow
        # ✅ Best Practice: Use of os.path.join for path construction improves cross-platform compatibility.
        is_optimal = self.results.empty or loss < self.best_score

        # ⚠️ SAST Risk (Low): Potential IndexError if worker_search_queue is empty
        # save the first model
        if is_optimal:
            # ⚠️ SAST Risk (Low): Potential KeyError if param_name is not in global_hyperparam_df
            # Try saving first, before updating info
            if model is not None:
                # ✅ Best Practice: Use logging instead of print for better control over output
                # 🧠 ML Signal: Loading serialized hyperparameters indicates a pattern of data persistence and retrieval.
                print("Optimal model found, updating")
                # 🧠 ML Signal: Worker-specific search queue suggests distributed processing pattern.
...
class DistributedHyperparamOptManager(HyperparamOptManager):
    ...
    # 🧠 ML Signal: Regenerating data when file is not found
    def __init__(
        self,
        param_ranges,
        fixed_params,
        # 🧠 ML Signal: Usage of a method to generate hyperparameter combinations, indicating a machine learning workflow.
        root_model_folder,
        # ⚠️ SAST Risk (Low): Use of print statements for logging, which may not be suitable for production environments.
        # ✅ Best Practice: Consider using a logging framework instead of print for better control over logging levels and outputs.
        worker_number,
        ...
        # 🧠 ML Signal: Creating a DataFrame to store hyperparameter combinations
        """

        # 🧠 ML Signal: Resets internal state, indicating a pattern of state management
        ...
        # ✅ Best Practice: Use of np.ceil to ensure all iterations are covered by workers
        # ✅ Best Practice: Use of np.concatenate and np.tile for efficient array creation
        # ⚠️ SAST Risk (Low): Potential IndexError if len(output) > len(worker_idx)
