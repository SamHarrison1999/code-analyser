...
def calculate_top(clear_em=True):
    ...
    while True:
        ...
        # ✅ Best Practice: Use of a scheduler to run tasks at specific times
        top_down_entity_ids = get_top_down_today()
        if top_down_entity_ids:
            # ⚠️ SAST Risk (Low): Potential for blocking the main thread
            # 🧠 ML Signal: Use of a scheduler to automate task execution
            build_stock_pool_and_tag_stats(
