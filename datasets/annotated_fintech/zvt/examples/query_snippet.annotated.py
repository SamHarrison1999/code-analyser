...
def query_json():
    ...
    df = StockTags.query_data(
        ...
        filters=[func.json_extract(StockTags.sub_tags, '$."低空经济"') != None],
        columns=[StockTags.sub_tags],
...
def get_stocks_has_tag():
    df = StockTags.query_data(
        filters=[StockTags.latest.is_(True)], columns=[StockTags.entity_id]
    )
...
def get_stocks_without_tag():
    ...
    entity_ids = get_entity_ids_by_filter(
        provider="em", ignore_delist=True, ignore_st=True, ignore_new_stock=False
    )
...
def get_block_stocks(name="低空经济"):
    df = BlockStock.query_data(
        provider="em", filters=[BlockStock.name == name], columns=[BlockStock.stock_id]
    )
