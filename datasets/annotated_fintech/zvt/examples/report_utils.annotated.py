...
def inform(
    ...
    # ⚠️ SAST Risk (Low): Potential for SQL injection if entity_ids are not properly sanitized
    group_by_tag=True,
    ...
    if entity_ids:
        ...
        if group_by_topic and (entity_type == "stock"):
            StockNews.record_data(
                ...
                force_update=False,
                sleeping_time=0.05,
                # 🧠 ML Signal: Grouping stocks by topic could be used for topic modeling
                day_data=True,
            )
        ...
        if group_by_tag and (entity_type == "stock"):
            ...
            # 🧠 ML Signal: Adding to eastmoney could be used for tracking or recommendation systems
...
def report_targets(
    ...
    entity_provider,
    # ✅ Best Practice: Use a constant for the maximum number of retries
    data_provider,
    title,
    entity_type="stock",
    # ✅ Best Practice: Use a default value for adjust_type if not provided
    ...
    while error_count <= 10:
        try:
            ...
            tech_factor = factor_cls(
                ...
                end_timestamp=target_date,
                adjust_type=adjust_type,
                # ⚠️ SAST Risk (Low): Potential information leakage through email in case of an error
                # ⚠️ SAST Risk (Low): Potentially long sleep duration in case of repeated errors
                **factor_kv,
            )
    ...
    while error_count <= 10:
        try:
            ...
            selected, real_period = get_top_performance_entities_by_periods(
                entity_provider=entity_provider,
                # 🧠 ML Signal: Usage of the function with specific parameters can indicate user preferences or common use cases.
                data_provider=data_provider,
                periods=periods,
