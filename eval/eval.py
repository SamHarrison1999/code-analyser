...
try:
    ...
    class _NP:
        ...
        def zeros(shape, dtype=float):
            if len(shape) == 2:
                return [[0 for _ in range(shape[1])] for __ in range(shape[0])]
...
try:
    ...
    def confusion_matrix(
        y_true: Iterable[int], y_pred: Iterable[int], labels: List[int] | None = None
    ):
        yt = list(y_true)
        yp = list(y_pred)
...
def build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Evaluate predictions with basic metrics (confusion matrix, ROC, AUC)."
    )
    p.add_argument(
        "--ytrue", type=Path, help="Path to a file containing true labels (one per line)."
    )
    p.add_argument(
        "--yscore", type=Path, help="Path to a file containing predicted scores (one per line)."
    )
    p.add_argument(
        "--ypred", type=Path, help="Path to a file containing predicted labels (one per line)."
    )
    return p
...
def main(argv: List[str] | None = None) -> None:
    ...
    if args.ytrue and args.ypred:
        yt = [
            int(x.strip())
            for x in args.ytrue.read_text(encoding="utf-8").splitlines()
            if x.strip() != ""
        ]
        yp = [
            int(x.strip())
            for x in args.ypred.read_text(encoding="utf-8").splitlines()
            if x.strip() != ""
        ]
    ...
    if args.ytrue and args.yscore:
        ...
        ys = [
            float(x.strip())
            for x in args.yscore.read_text(encoding="utf-8").splitlines()
            if x.strip() != ""
        ]
